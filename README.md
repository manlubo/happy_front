...
- **아이콘 관리**
  - components/common/Icons.tsx에서 아이콘을 중앙화하여 관리

- **Throttle 적용(일정시간 동안 동일한 요청 방지)**
  - 메일전송, 인증문자 전송 등 동일한 요청이 발생하는 것을 방지하고, 해당 시간동안 버튼을 비활성화하여 쿨다운 타이머 표시
  - useCooldown 커스텀 훅을 통해 쿨다운 시간 관리

- **커스텀 훅관리**
  - 비즈니스 로직(`mutation`, `query`)을 커스텀 훅으로 분리하여 재사용성을 높이고, 코드의 가독성을 높임
